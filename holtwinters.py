...
class HoltWintersEstimator(Estimator):
	"""Represents a Holt-Winters estimator
	Attributes:
		number_of_seasons: Number of seasons in the time series. If you have 3
		years of observations, with month by month data, number_of_seasons=3.

		season_size: Indicates the number of observations in each season. For
		example, if you have 3 years of observation, with month by month data,
		season_size=12, which is the number of months in a year.

		seasonal_factor: Indicates the seasonal factor of each observation on
		your data set.

		average_component: Indicates the average component in the Holt-Winters
		model.

		tendence_component: Indicates the tendence component in the Holt-Winters
		model.

		seasonal_component: Indicates the seasonal component in the Holt-Winters
		model. It is a matrix whose first index is the seasonal index of a given
		time, and the second index is the season itself.
	"""
	...
	def seasonOfTime(self, time):
		"""	Returns the season which holds the data at a given time, from 1 to
		number_of_seasons
		"""
	...
	def seasonalFactor(self, time):
		""" Calculates the seasonal factor for each historical data in the time
		series.
		"""
	...
	def estimate(self, time, base_time):
		""" Estimate the value of the function on time, based on the base_time
		observation. Often base_time is time-1.
		"""
...
hwe = HoltWintersEstimator(time_series_soda, 3, 12) #36
print(hwe.estimate(37, 36))
