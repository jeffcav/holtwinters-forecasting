...
class HoltWintersEstimator(Estimator):
    ...
    def estimate(self, time, base_time):
        ...
        estimation = (self.average_component[base_time] + self.tendence_component[base_time] * time) * self.getSeasonalComponent(time - self.season_size)
        return estimation
    ...
    def calculateComponents(self, alpha, beta, gamma):
        for time in range(1, len(self.time_series) + 1):
            ...
            season = self.seasonOfTime(time)
            self.insertSeasonalComponent(time, season, seasonal_component)
